    ...
    <div className="inspector-control">
            ...
            >
              {jobs.map((j) => (
                <option key={j.job_id} value={j.job_id}>
                  {j.job_id.slice(0, 8)}… · {j.status} · {j.pages_crawled}/{j.pages_discovered}
                </option>
              ))}
            </select>
          </label>
      ...
      {job && (
        <div className="inspector-stats">
          <span>
            وضعیت: <strong className={`status-pill status-${job.status}`}>{job.status}</strong>
          </span>
          ...
          <span>شروع: {fmtTime(job.started_at)}</span>
          {job.finished_at && <span>پایان: {fmtTime(job.finished_at)}</span>}
